...
const DEFAULTS = {
  ...
  ignoreThreeFiberTags: false,
};

const THREE_FIBER_TAGS = [
  'object3D',
  'audioListener',
  'positionalAudio',
  'mesh',
...
function isThreeFiberTag(tag) {
  return THREE_FIBER_TAGS.indexOf(tag) !== -1 
}
...
module.exports = {
  ...
  create(context) {
    ...
    function getIgnoreThreeFiberTags() {
      return (context.options[0] && typeof context.options[0].ignoreThreeFiberTags !== 'undefined')
        ? !!context.options[0].ignoreThreeFiberTags
        : DEFAULTS.ignoreThreeFiberTags;
    }
    ...
    return {
      JSXAttribute(node) {
        ...
        // Ignore react-three-fiber defined tags
        if (getIgnoreThreeFiberTags() && isThreeFiberTag(node)) {
          return;
        }
